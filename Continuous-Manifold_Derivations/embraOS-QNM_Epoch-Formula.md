...
> **Its operational shadow — the Classical Approximation.** This file is the *speculative* continuous ideal: ψ is **inherent** (the projection `P_ψ`), nothing is checked, and nothing is built — so it **cannot fail**. Its buildable counterpart on **classical hardware**, where `P_ψ` is approximated by a *checked* carried latch (`σ_verify` **retained**), is a separate **discrete / operational** derivation — [embraOS-QNM (Classical Approximation): The Constructed Epoch](../Discrete_Derivations/embraOS-QNM-Classical_Epoch-Formula.md) (`README.md` §9; legend §15). That one was built and **failed honestly** — four pre-registered reader families returned generic and its program was sunset at v0.4.0 (2026-07-16). Its pivot target, [embraOS-QNM-Core: The Conserved Epoch](./embraOS-QNM-Core_Epoch-Formula.md) (`README.md` §7; legend §16), sits in *this* folder: a **running** core where ψ is a **conserved charge** — a Casimir of the bracket — neither a projection nor a check, so it is *not* an implementation of this document's `P_ψ` either; **this** continuous formulation stays *theoretical* (see *Current Status*).
...
- [`embraOS-QNM (Classical Approximation)`](../Discrete_Derivations/embraOS-QNM-Classical_Epoch-Formula.md) — the **discrete / operational** Classical Approximation of this continuous ideal: `P_ψ` becomes a *checked* carried latch (`σ_verify` retained), a constructed instance that can fail (`README.md` §9, legend §15) — **a relic** since 2026-07-16
- [`embraOS-QNM-Core: The Conserved Epoch`](./embraOS-QNM-Core_Epoch-Formula.md) — the **running** successor in this folder: ψ *conserved* as a Casimir coordinate block (not projected, not checked); it does not realise this document's `P_ψ` (`README.md` §7, legend §16)
